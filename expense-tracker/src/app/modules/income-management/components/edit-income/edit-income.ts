...
categories:any
  ...
  paymentModes:any=[{id:1,name:"Cash",value:"cash"},
    ...
    readonly dialogRef = inject(MatDialogRef<EditIncome>);

  constructor(private fb: FormBuilder,private commonService:CommonServices,@Inject(MAT_DIALOG_DATA) public data: any
) {
  ...
  patchformdata()
  {
    ...
    this.incomeForm.patchValue({
      ...
      incomeDate: this.data.income.incomeDate,
      source: this.data.income.source,
      amount: this.data.income.amount,
      paymentMode: this.data.income.paymentMode
    });
  }
  }
  onSubmit(): void {
    ...
    this.commonService.updateIncome(this.data.income.incomeId,formData).subscribe(res=>
      {
        console.log("income updated successfulyy",formData)
